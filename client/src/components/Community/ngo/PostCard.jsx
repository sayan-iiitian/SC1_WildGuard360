...
const ReplyCard = ({post ,reply, user, handleLike }) => {
  return (
...
const CommentForm = ({ user, id, replyAt, getComments }) => {
  ...
  const onSubmit = async (data) => {
    ...
    setErrMsg("");
    try{
      const URL=!replyAt?"/posts/comment/"+id
      ...
      <div className='flex items-end justify-end pb-2'>
        {loading ? (
          ...
          <CustomButton
            title='Submit'
            type='submit'
            containerStyles='bg-[#0F8636] text-white py-1 px-3 rounded-full font-semibold text-sm'
          />
        )}
      </div>
    </form>
...
const PostCard = ({ post, user, deletePost, likePost }) => {
  ...
  //   await likePost(localStorage.getItem("postID"));
  ...
  const handleLike = async () => {
    if (post && post._id) {
      try {
        ...
        // Like the post and get the result
        const result = await likePost(post._id, token);
      ...
      console.error("Post data is not available or does not have an _id.");
    }
  };
  ...
  return (
    <div className='mb-2 bgcolortailwind p-4 rounded-xl'>
